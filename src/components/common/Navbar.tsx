...
export default function Navbar() {
  ...
  // Don't render navbar for admin pages
  if (isAdminRoute) {
    return null;
  }
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-[100] bg-neutral-950/80 backdrop-blur-md border-b border-white/10 px-6 md:px-12 py-4 flex items-center justify-between text-white">
      ...
      <Link
        href="/"
        className="text-2xl font-black tracking-tighter hover:opacity-80 transition"
      >
        Project Atlas
      </Link>
      ...
      <div className="hidden md:flex items-center space-x-8 text-sm uppercase tracking-wider">
        <Link
          href="/rent-a-car"
          className={linkStyle("/rent-a-car")}
        >
          Rent a Car
        </Link>
        ...
        <Link
          href="/book-a-flight"
          className={linkStyle("/book-a-flight")}
        >
          Book a Flight
        </Link>

        <Link
          href="/book-a-transfer"
          className={linkStyle("/book-a-transfer")}
        >
          Book a Transfer
        </Link>
      </div>
